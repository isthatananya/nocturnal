...
export function statusConfig(s: FactorStatus) {
  return {
    ...
    good:      { icon: CheckCircle,   color: 'text-emerald-300', bg: 'bg-emerald-500/6  border-emerald-500/15', bar: 'bg-emerald-300', label: 'Good' },
    ...
    <div className={`rounded-2xl border p-5 ${cfg.bg}`}>
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-zinc-100 text-sm">{f.label}</span>
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${cfg.color} bg-black/20`}>{f.statusLabel}</span>
        </div>
        <span className="text-sm font-mono text-zinc-400 shrink-0">{f.pts}<span className="text-zinc-600">/{f.max}</span></span>
      </div>
      ...
      <p className="text-sm text-zinc-300 mb-2">{f.verdict}</p>

      {f.tip && (
        <div className="flex items-start gap-2 mt-3 pt-3 border-t border-white/5">
          <Lightbulb size={13} className="text-white/40 mt-0.5 shrink-0" />
          <p className="text-xs text-zinc-400 leading-relaxed">{f.tip}</p>
        </div>
      )}
    </div>
  )
