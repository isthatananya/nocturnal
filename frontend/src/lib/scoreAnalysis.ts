...
export interface FactorDetail {
  ...
  /**
   * Raw-point gain (out of 100) achievable by following the top recommended
   * action on this factor. Multiply by 6 for CIBIL-scale impact. Null when
   * the factor is already maxed or the gain is non-quantifiable.
   */
  potentialGain: number | null
}

/** A single concrete action a borrower can take to improve their score. */
export interface ImprovementAction {
  factorKey: string
  factorLabel: string
  action: string           // imperative, specific, ≤120 chars
  rawPointGain: number     // out of 100
  cibilPointGain: number   // rawPointGain × 6
  difficulty: 'easy' | 'medium' | 'hard'
  timeline: string         // 'next cycle' | 'within 6 months' | 'within 12 months' | …
}
...
export interface ScoreAnalysis {
  ...
  topImprovements: ImprovementAction[]   // up to 3, sorted by CIBIL impact desc
}
...
function paymentFactor(bd: Breakdown, inp?: Partial<FeatureVector>): { detail: FactorDetail; actions: ImprovementAction[] } {
  ...
  if (settled) {
    ...
    if (gap > 0) actions.push({
      factorKey: 'payment', factorLabel: 'Payment History',
      action: 'Apply for "no-due certificate" conversion on the settled account after 24 months of clean conduct',
      rawPointGain: Math.min(gap, 20), cibilPointGain: Math.min(gap, 20) * 6,
      difficulty: 'hard', timeline: '24+ months',
    })
    ...
    actions.push({
      factorKey: 'payment', factorLabel: 'Payment History',
      action: 'Clear the 90+ DPD account now; even partial payment stops the delinquency clock',
      rawPointGain: gap, cibilPointGain: gap * 6,
      difficulty: 'hard', timeline: '12 months to fully recover',
    })
    ...
    actions.push({
      factorKey: 'payment', factorLabel: 'Payment History',
      action: 'Set up NACH auto-debit on every EMI account and keep 2× the largest EMI as float',
      rawPointGain: Math.min(gap, 17), cibilPointGain: Math.min(gap, 17) * 6,
      difficulty: 'easy', timeline: 'next billing cycle',
    })
    ...
    actions.push({
      factorKey: 'payment', factorLabel: 'Payment History',
      action: 'Enable auto-debit on all EMIs to recover from missed payments — clears after 12 clean months',
      rawPointGain: gap, cibilPointGain: gap * 6,
      difficulty: 'easy', timeline: '12 months',
    })
    ...
    actions.push({
      factorKey: 'payment', factorLabel: 'Payment History',
      action: 'Maintain a perfect payment record — the existing delay ages off after 12 months',
      rawPointGain: gap, cibilPointGain: gap * 6,
      difficulty: 'easy', timeline: '12 months',
    })
  } else {
  ...
  return {
    detail: { key: 'payment', label: 'Payment History', pts: bd.payment_pts, max: 35, pct, status: s, statusLabel: statusLabel(s), what: 'EMI repayment discipline: DPD, missed payments, write-offs', verdict, tip, potentialGain: gap > 0 ? gap : null },
    actions,
  }
}

function foirFactor(bd: Breakdown, inp?: Partial<FeatureVector>): { detail: FactorDetail; actions: ImprovementAction[] } {
  ...
  let tip: string | null = null
  const actions: ImprovementAction[] = []
  const gap = 25 - bd.foir_pts

  // Backend FOIR bins: ≤20%→25, ≤30%→20, ≤40%→14, ≤50%→8, ≤60%→3, else 0.
  // Target the next bracket boundary for a concrete reduction figure.
  const nextBracket = (cur: number): { target: number; pts: number } | null => {
    if (cur > 60) return { target: 60, pts: 3 }
    if (cur > 50) return { target: 50, pts: 5 }
    if (cur > 40) return { target: 40, pts: 6 }
    if (cur > 30) return { target: 30, pts: 6 }
    if (cur > 20) return { target: 20, pts: 5 }
    return null
  }
  ...
  if (foirPct !== null && emi && inc) {
    const next = nextBracket(foirPct)
    if (next) {
      const targetEmi = Math.round(inc * (next.target / 100))
      const reduce = Math.max(0, emi - targetEmi)
      actions.push({
        factorKey: 'foir', factorLabel: 'EMI Burden (FOIR)',
        action: `Reduce monthly EMIs by ₹${reduce.toLocaleString('en-IN')} (prepay the highest-rate loan first) to drop FOIR from ${foirPct}% → ${next.target}%`,
        rawPointGain: next.pts, cibilPointGain: next.pts * 6,
        difficulty: foirPct > 50 ? 'hard' : 'medium',
        timeline: foirPct > 50 ? '6–12 months' : '3–6 months',
      })
    }
  } else if (gap > 0) {
    actions.push({
      factorKey: 'foir', factorLabel: 'EMI Burden (FOIR)',
      action: 'Reduce existing EMIs so they stay below 30% of monthly income',
      rawPointGain: Math.min(gap, 8), cibilPointGain: Math.min(gap, 8) * 6,
      difficulty: 'medium', timeline: '3–6 months',
    })
  }

  return {
    detail: { key: 'foir', label: 'EMI Burden (FOIR)', pts: bd.foir_pts, max: 25, pct, status: s, statusLabel: statusLabel(s), what: 'Monthly EMI obligations as % of take-home income', verdict, tip, potentialGain: gap > 0 ? gap : null },
    actions,
  }
}

function historyFactor(bd: Breakdown, inp?: Partial<FeatureVector>): { detail: FactorDetail; actions: ImprovementAction[] } {
  ...
  if (months !== null && months < 6) {
    ...
    actions.push({
      factorKey: 'history', factorLabel: 'Credit History Age',
      action: 'Open a secured credit card (against an FD) and use it for small monthly bills',
      rawPointGain: 2, cibilPointGain: 12,
      difficulty: 'easy', timeline: '6 months',
    })
    ...
    actions.push({
      factorKey: 'history', factorLabel: 'Credit History Age',
      action: 'Keep your oldest credit account open; closing it shortens average history age',
      rawPointGain: Math.min(gap, 3), cibilPointGain: Math.min(gap, 3) * 6,
      difficulty: 'easy', timeline: '12 months',
    })
  ...
  return {
    detail: { key: 'history', label: 'Credit History Age', pts: bd.history_pts, max: 15, pct, status: s, statusLabel: statusLabel(s), what: 'Age of your oldest credit account', verdict, tip, potentialGain: gap > 0 ? gap : null },
    actions,
  }
}

function mixFactor(bd: Breakdown, inp?: Partial<FeatureVector>): { detail: FactorDetail; actions: ImprovementAction[] } {
  ...
  let tip: string | null = null
  const actions: ImprovementAction[] = []
  const gap = 15 - bd.credit_mix_pts

  // Backend util bins: ≤20%→6, ≤30%→5, ≤50%→3, ≤75%→1, else 0
  if (utilPct !== null && utilPct > 75) {
    ...
    actions.push({
      factorKey: 'mix', factorLabel: 'Credit Mix & Utilization',
      action: `Pay down CC balance below 30% utilization before the next statement date (currently ${utilPct}%)`,
      rawPointGain: 4, cibilPointGain: 24,
      difficulty: 'medium', timeline: 'next billing cycle',
    })
    ...
    actions.push({
      factorKey: 'mix', factorLabel: 'Credit Mix & Utilization',
      action: `Request a credit limit increase (without spending more) to drop utilization from ${utilPct}% → under 30%`,
      rawPointGain: 3, cibilPointGain: 18,
      difficulty: 'easy', timeline: 'next billing cycle',
    })
    ...
    actions.push({
      factorKey: 'mix', factorLabel: 'Credit Mix & Utilization',
      action: `Pay CC balance mid-cycle to drop utilization from ${utilPct}% to under 30%`,
      rawPointGain: 2, cibilPointGain: 12,
      difficulty: 'easy', timeline: 'next billing cycle',
    })
    ...
    actions.push({
      factorKey: 'mix', factorLabel: 'Credit Mix & Utilization',
      action: 'Add one secured product (secured CC or small gold loan) to diversify credit mix',
      rawPointGain: Math.min(gap, 4), cibilPointGain: Math.min(gap, 4) * 6,
      difficulty: 'easy', timeline: '1–2 weeks',
    })
    ...
    actions.push({
      factorKey: 'mix', factorLabel: 'Credit Mix & Utilization',
      action: 'Add one secured loan (gold loan is fastest at 1–2 days) to balance the unsecured-only mix',
      rawPointGain: 2, cibilPointGain: 12,
      difficulty: 'easy', timeline: '1–2 weeks',
    })
  } else {
  ...
  return {
    detail: { key: 'mix', label: 'Credit Mix & Utilization', pts: bd.credit_mix_pts, max: 15, pct, status: s, statusLabel: statusLabel(s), what: 'Credit card utilization + diversity of loan types', verdict, tip, potentialGain: gap > 0 ? gap : null },
    actions,
  }
}

function inquiryFactor(bd: Breakdown, inp?: Partial<FeatureVector>): { detail: FactorDetail; actions: ImprovementAction[] } {
  ...
  if (n !== null && n > 4) {
    ...
    actions.push({
      factorKey: 'inquiry', factorLabel: 'New Applications',
      action: 'Stop applying for new credit for the next 6 months — use soft-pull eligibility checkers instead',
      rawPointGain: gap, cibilPointGain: gap * 6,
      difficulty: 'easy', timeline: '6 months',
    })
  } else if (n !== null && n > 2) {
    ...
    actions.push({
      factorKey: 'inquiry', factorLabel: 'New Applications',
      action: 'Avoid new loan applications for 4–6 months — inquiries age off the report progressively',
      rawPointGain: Math.min(gap, 5), cibilPointGain: Math.min(gap, 5) * 6,
      difficulty: 'easy', timeline: '4–6 months',
    })
  } else if (n !== null && n > 0) {
    ...
    if (n > 1) actions.push({
      factorKey: 'inquiry', factorLabel: 'New Applications',
      action: 'Hold off on new applications for 3 months so existing inquiries age off',
      rawPointGain: 2, cibilPointGain: 12,
      difficulty: 'easy', timeline: '3 months',
    })
  } else {
  ...
  return {
    detail: { key: 'inquiry', label: 'New Applications', pts: bd.inquiry_pts, max: 10, pct, status: s, statusLabel: statusLabel(s), what: 'Hard credit inquiries in the last 6 months', verdict, tip, potentialGain: gap > 0 ? gap : null },
    actions,
  }
}
...
export function analyseReport(bd: Breakdown, inputs?: Partial<FeatureVector>): ScoreAnalysis {
  ...
  const factors: FactorDetail[] = evals.map(e => e.detail)
  const allActions = evals.flatMap(e => e.actions)

  // Rank by CIBIL impact desc, then by ease (easy > medium > hard) to surface
  // the wins that are both big and reachable.
  const difficultyRank = { easy: 0, medium: 1, hard: 2 } as const
  const topImprovements = allActions
    .sort((a, b) =>
      b.cibilPointGain - a.cibilPointGain ||
